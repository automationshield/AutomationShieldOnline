...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($experiment === 'PID') {
        ...
        if ($id && $timer2 > 0) {
            ...
            $sql = "UPDATE FloatShield SET Experiment = ?, START = ?, Kp = ?, Ti = ?, Td = ?, r1 = ?, r2 = ?, r3 = ?, r4 = ?, r5 = ?, r6 = ?, T = ? WHERE arduino_online_id = ?";
            $stmt = $db->prepare($sql);

            if ($stmt) {
                logDebug("Prepared update query for FloatShield");
                $stmt->bind_param(
                    'ssddddddddddi',
        ...
        if ($id && $timer > 0) {
            $sql = "UPDATE FloatShield SET Experiment = ?, START = ?, TIMER = ? WHERE arduino_online_id = ?";
            $stmt = $db->prepare($sql);
        ...
        if ($id && $timer > 0) {
            $sql = "UPDATE FloatShield SET Experiment = ?, START = ?, TIMER = ?, r1 = ?, r2 = ?, r3 = ?, r4 = ?, r5 = ?, r6 = ? WHERE arduino_online_id = ?";
            $stmt = $db->prepare($sql);
