...
selectElement.addEventListener('change', function () {
  ...
  // ***************************** DISCONNECT CHECK **************************************
    ...
    function checkShieldStatus() {
        ...
        fetch(`AeroShield_DataCheck.php?id=${encodeURIComponent(id)}`)
            .then(response => response.json())
            .then(data => {
...
async function fetchDataAndUpdateGraph() {
    try {
        const response = await fetch(`AeroShield_fetch.php?id=${encodeURIComponent(id)}`);
